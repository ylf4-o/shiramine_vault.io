...
- 自動保存
- ノート一覧
- Decision Log（意思決定ログ）のCRUD、カテゴリ・タグ絞り込み、関連ノートリンク
- タイトル・本文・タグ検索
- PC三ペインUI
- スマホ向けタブ切り替えUI
- `[[ノート名]]` の検出
...
- Supabase `decision_logs` テーブルへの任意同期
...
日々の小さな意思決定を、あとから価値観・行動傾向・発信ネタへ変換するための記録欄です。
...
初期カテゴリは `仕事`、`お金`、`健康`、`人間関係`、`創作`、`生活`、`遊び`、`その他` です。評価は `正解`、`微妙`、`失敗`、`保留` です。

PCでは左サイドバーの `意思決定ログ`、スマホでは下部ナビの `意思決定` から開きます。ログは日付順で表示し、カテゴリとタグで絞り込めます。
...
7. `notes`、`decision_logs`、`tags`、`note_tags`、`links`、`note_versions` のRLSが有効であることを確認する。
...
- Decision Logも `decision_logs` に同期
